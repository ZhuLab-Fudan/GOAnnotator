...
class PubRetriever:
    ...
    def __init__(
        ...
        Args:
            ...
            input (str): Path to input protein ID file.
            output (str): Path to output result file.
    ...
    def fine_grained_retrieval(self, protein_metadata: Dict) -> List[Dict[str, str]]:
            ...
            for item in names:
                if item.get("Name"):
                    ...
                    if len(scores) >= self.k:
                        query = (
                            (filter_text(item["Name"]) + " ") * 4
                            + " "
                            + filter_species(protein_metadata["species"])
                        )
                        scores = self.meta_FGR(
                            query,
                            scores,
        ...
        if len(scores) >= self.k:
            ...
            res = []
            for pmid in pmids:
                doc = json.loads(self.searcher.doc(pmid).raw())
                res.append(
                    {
                        "pmid": doc["id"],
                        "text": doc["contents"],
    ...
    def hybrid_retrieval(self, proid: str, protein_metadata: Dict) -> List[Dict[str, str]]:
        """
    ...
    def retrieve_and_rerank(
        ...
        """
        hybrid_retrieval_results = self.hybrid_retrieval(proid, protein_metadata)

        if len(hybrid_retrieval_results) < 1:
            print(proid)
            return []
    ...
    def process_proteins(self):
        ...
        with open(self.input) as input_file:
            with open(self.output, "w") as output_file:
                proids = [line.strip() for line in input_file.readlines()]
                ...
                for proid in tqdm(proids, desc="Processing proteins"):
                    ...
                    documents = self.retrieve_and_rerank(proid, protein_metadata)
